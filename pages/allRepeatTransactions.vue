      ...
      <div class="content">
        <div class="panel">
          ...
          <div class="flex justify-between mr-4">
            ...
            <div>
              <button class="btn btn-ghost btn-xs" @click="$store.commit('showModal', {'name': 'new-repeat-transaction'})">+</button>
          ...
          <table class="table w-full table-compact table-zebra">
            ...
            <tr v-for="transaction in transactions">
              ...
              <td>{{getDate(transaction.lastRepeat.seconds)}}</td>
              <td>{{getDate(transaction.nextRepeat.seconds)}}</td>
              <td>{{getAccountName(transaction.account)}}</td>
              <td>{{transaction.description}}</td>
              <td>
                <button @click="$store.commit('showModal', {'name': 'repeat-transaction-editor', data: transaction})" class="btn btn-ghost p-1">
...
export default {
  ...
  methods: {
    ...
    loadTransactions(page){
      ...
      fetch(this.$axios.defaults.baseURL + "api/transactions/repeat?token=" + session + "&count=25" + "&offset=" + ((page - 1) * 25))
  ...
  beforeMount() {
    ...
    this.$store.commit("setModalHideEvent", {
      name: "repeat-transaction-editor",
      func: () => {
        this.loadTransactions(this.page)
      }
    })
